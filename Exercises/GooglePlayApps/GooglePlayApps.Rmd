...
```
**This is the 15.6%, it is too high... I am going to fill it with the mean anyway, depeding on the analysis I may change the mean to match the mean per app category** 
...
text_temp <-  size_summary$Mean[str_detect(size_summary$Category,"ART_AND_DESIGN")]
class(apps_dataset$CategoryTemp)
text_temp

apps_dataset <- apps_dataset %>%
  mutate(CategoryTemp = as.character(Category)) %>%
   mutate(SizeNumeric = case_when(
       str_detect(Size,"^Varies with device$") ~ size_summary$Mean[str_detect(size_summary$Category,CategoryTemp)],
       TRUE ~ SizeNumeric))
# 
# # Validating
# apps_dataset %>%
